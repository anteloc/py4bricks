...
from py4bricks.utils import class_name
from pathlib import Path


def gen_colours(parts: Parts, library_path: Path):
...
def colours_module_content(parts: Parts):
...
def get_c_dict(colour: Colour) -> dict:
    """Get a dict from a Colour object."""
    return {
        "code": colour.code,
        "full_name": colour.name,
        "name": class_name(colour.name or "UNKNOWN"),
        "alpha": colour.alpha,
        "rgb": colour.rgb,
